    ...
    local data_send =
      ...
      function()
        if self.sock_udp then
		  ...
		  local slip = LoGetAngleOfSideSlip()
...
-------------------------------------------------------------------------------------------------------------------------------------------------------
          ...
          -- Common variables
          stringToSend = string.format("T=%.3f;N=%s;SelfData=%s;%s;EngRPM=%s;ACCs=%s;Gun=%s;Wind=%s;VlctVectors=%s;altASL=%.2f;altAgl=%.2f;AoA=%.2f;IAS=%.2f;TAS=%.2f;WeightOnWheels=%s;Flares=%s;Chaff=%s;PayloadInfo=%s;Mach=%.4f;MechInfo=%s;slip=%.2f",               
            t,
            obj.Name,
            myselfData,
            stringToSend,
            ...
            CannonShells,
            windVelocityVectors,
            velocityVectors, altAsl, altAgl, aoa, IAS, tas, WoW, CM.flare, CM.chaff, PayloadInfo, M_number, mech, slip)
...
do
  local zSimLuaExportStart = LuaExportStart
  LuaExportStart = function()
    f_zTelem:Start()
    if zSimLuaExportStart then
      zSimLuaExportStart()
    end
  end
end
...
do
  local zSimLuaExportAfterNextFrame = LuaExportAfterNextFrame
  LuaExportAfterNextFrame = function()
    f_zTelem:AfterNextFrame()
    if zSimLuaExportAfterNextFrame then
      zSimLuaExportAfterNextFrame()
    end
  end
end
...
do
  local zSimLuaExportStop = LuaExportStop
  LuaExportStop = function()
    f_zTelem:Stop()
    if zSimLuaExportStop then
      zSimLuaExportStop()
    end
  end
end
...
do
  local zSimLuaExportBeforeNextFrame = LuaExportBeforeNextFrame
  LuaExportBeforeNextFrame = function()
    f_zTelem:BeforeNextFrame()
    if zSimLuaExportBeforeNextFrame then
      zSimLuaExportBeforeNextFrame()
    end
  end
end
